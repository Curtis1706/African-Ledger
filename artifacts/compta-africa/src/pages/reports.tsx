import { useGetMonthlyReport, useGetReportSummary } from "@workspace/api-client-react";
...
export default function Reports() {
  ...
  const { data: reports, isLoading: isLoadingReports } = useGetMonthlyReport({ year: currentYear });
        ...
        <Card>
          ...
          <CardContent>
            {isLoadingSummary ? <Skeleton className="h-8 w-32" /> : (
              <div className="text-xl font-bold capitalize">{summary?.bestMonth || '-'}</div>
            )}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {isLoadingSummary ? <Skeleton className="h-8 w-32" /> : (
              <div className="text-xl font-bold capitalize">{summary?.worstMonth || '-'}</div>
            )}
          </CardContent>
        </Card>
      </div>
          ...
          {isLoadingReports ? (
            ...
            <div className="h-[400px] w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={reports || []} margin={{ top: 20, right: 20, left: 20, bottom: 5 }}>
